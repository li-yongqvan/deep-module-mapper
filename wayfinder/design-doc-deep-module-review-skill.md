...
- **主图质量档位的实测预期**【2026-09-05 用户拍板，#24 v2 遗留项消解】：多模块仓库（依赖成簇，如本仓库 8 模块）主图默认预期 = **standard 档**。实测 archify 正交路由器为障碍感知，本仓库三种布局均无法通过 showcase 检查（ambiguous-corridor / edge-through-node）；管线按上文单次降档规则接受 standard 并缓存。showcase 仍是目标档位（IR 仍 aims high；只过 standard 的布局不钉死缓存、每次重求解），但应预期密集多模块仓库的主图连线观感为 standard 档——不影响图内任何事实数据（面板函数集/调用边与 `intra` 逐条一致已由 e2e 固化）；下钻面板固定 standard（assemble.py `WORKFLOW_QUALITY`），不受主图档位影响。
