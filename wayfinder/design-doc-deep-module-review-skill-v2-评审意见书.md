...
- [x] **F1**：§14/§16 增补 test_scan_codebase.py:22 的同步修改说明；schema.json 的 `intra` properties 与 `required` 去留定案；README/SKILL.md 键数表述列入改动清单；5 键 golden 单测入 §17。
- [x] **F2**：§14 增补同名遮蔽消歧规则与属性调用取舍；§17 补 shadowing 场景单测。
- [x] **F3**：§15 增补模块 id → archify id 的确定性映射规则 + 碰撞断言；§17 补碰撞测试。
- [x] **F4**：Q5 落裁决（固定 seed / 布局缓存 / 进程内几何校验），§15"确定性"措辞修正。
- [x] **F5**：V2-D10 降级契约写死（产物形态 + 明示机制 + node 探测）；§17 补降级 e2e 两例。
- [x] **F6**（建议）：§14 归属表补全（嵌套/lambda/条件 def/重名/顶层调用）。
- [x] **F7**（建议）：§12 明确 map.html 交付方式。
- [x] **F8**（建议）：assemble.py 样式兜底与 subprocess 编码单测。
- [x] 实现完成后：parser 测试全绿（含更新后的键断言与新增单测）、skill 单测全绿、e2e 对本仓库产出 map.html 且 7 面板函数/边数与 `intra` 一致、降级路径人工触发一次。
