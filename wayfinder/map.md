...
- GitHub issue #8（改写）: [Feature view: functional-atom grouping + Chinese descriptions](https://github.com/li-yongqvan/deep-module-mapper/issues/8) — 当前 active frontier ticket：视图层把文件级模块聚合为「功能原子」节点，显示中文功能描述；第一版用手工维护的功能清单顶替 AI 聚合。
- GitHub issue #10: [Recomposition layer: custom canvas with module-content editing](https://github.com/li-yongqvan/deep-module-mapper/issues/10) — 依赖 #8；用户在画布上把功能原子拖进/拖出模块 + 连依赖。
- GitHub issue #11: [AI aggregation: local model clusters files into functional atoms](https://github.com/li-yongqvan/deep-module-mapper/issues/11) — 依赖 #8；本地模型读文件内容自动判断功能原子，替代手工清单。
