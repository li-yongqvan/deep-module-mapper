...
- **#31 SKILL.md 宪法化瘦身**（2026-09-06 立项）— **active**：审计结论 = 当前约一半是"教 AI 怎么做"的 runbook（命令/输出表格/JSON 格式细节），必须留的是宪法（评审哲学/红线/产出契约）。方向 = 删 runbook + 脚本自描述化（analyze stdout 打印下一步指引），与「普查优先、遇障降级策展」一致，兼减双份维护负担。详见 issue #31。
