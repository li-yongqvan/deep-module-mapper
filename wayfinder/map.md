...
- **2026-09-06（skill 级设计原则，用户拍板）**：**「普查优先、遇障降级策展」**——普查（全量事实地图，条条带行号证据）是默认追求的最高标准；但遇到解析不了/渲染不动的部分**不卡死**：明确说「这里出现问题」，跳过它，交付能做到的部分，跳过项如实列出。与「宁缺勿幻」一脉相承（宁缺勿卡、宁缺勿假）。首个适用点 #27（布局搜索刹车 + 主图按包聚合）、#28（零模块守卫）；#30 草稿决策 0 已收录。对照教训：archify 原生玩法 = 策展（≤12 主节点 + 有界修补循环），我们此前把 124 节点全塞给它 + 无上限搜索，属于违反其两条原生约束的复合伤害（详见 #27 评论）。
...
- [robustness-three](wayfinder/resolution-28-comment.md) — **评审鲁棒性三项完成（PR #32，mergeCommit `b9010d3`，2026-09-06）**：① 零模块守卫——`analyze.py` 对零 `.py` 仓库明确报错 exit 1、零产物写出（#30 多语言前的短期替身），实测 ai-forum；② 星号导入诊断聚合——新诊断 `star_import_unresolved` 把含 `import *` 模块的引用侧未解析按每条星号导入聚合成一条（锚定 import 行 + 模块级计数 + ≤5 命名样本），措辞保留 may-come-from 诚实语义（静态无法归属，根治在 #29），QuickCut 586→4 / you-get 1626→105；③ 扫描警告隔离——模块体与字符串注解解析两处 `warnings.catch_warnings` 隔离，SyntaxWarning 不再泄漏 stderr。验证：135 测试全绿（+9 新）、golden 5 键逐字节一致、三狗粮仓库实测。**issue #28 经 `Closes #28` 自动关闭。**
...
- **#31 SKILL.md 宪法化瘦身**（2026-09-06 立项）— **active（handoff 已备：`wayfinder/handoff-issue-31-skill-constitutional-slimming.md`）**：审计结论 = 当前约一半是"教 AI 怎么做"的 runbook（命令/输出表格/JSON 格式细节），必须留的是宪法（评审哲学/红线/产出契约）。方向 = 删 runbook + 脚本自描述化（analyze stdout 打印下一步指引），与「普查优先、遇障降级策展」一致，兼减双份维护负担。详见 issue #31。
- **#29 星号导入符号解析**（2026-09-06 立项）— **active**：#28 子项 B 的根治姊妹票。两趟解析（Pass 1 建模块级公开符号表、Pass 2 展开星号导入），把静态可解的 unresolved_symbol 真正解析掉（you-get 1626 条 → 预期仅剩真动态注入）；真动态手法维持「宁缺勿幻」报诊断。建议先 #28 止血、再 #29 根治（架构级改动走设计文档+评审）。详见 issue #29。
- **#30 多语言前端架构**（2026-09-06 立项）— **active（大方向票）**：统一骨架（语言无关）+ 可插拔语言前端 ×N；Python 前端 = 现有 parser。已拍板决策 0「普查优先、遇障降级策展」为 skill 级总纲；候选方向含「AI 作为通用前端的混合架构」（AI 提取边标"未经逐条核验"）。需设计文档 + grilling 后才进实现。短期替代 = #28 零模块守卫 + 明示语言覆盖。详见 issue #30。
- **#27 v2 布局兜底搜索在 100+ 模块仓库不收敛**（2026-09-06 立项，狗粮暴露）— **active**：`to_archify.py` 爬山兜底搜索无候选上限/时间预算/进度输出；you-get 124 生产模块实测单核满载 17+ 分钟无结果（8 模块仓库秒级）。狗粮复盘：系违反 archify 原生约束（≤12 主节点 + 有界修补循环）的复合伤害；修法补充「主图按包聚合 + 搜索刹车（限时交卷+如实报告）」，见 #27 评论。详见 issue #27。
- **#28 已完成关闭**（2026-09-06）：PR #32 已合并（mergeCommit `b9010d3`），issue #28 经 `Closes #28` 自动关闭。三子项（零模块守卫 / 星号导入聚合 / 警告隔离）见 Decisions。
