"""Lightweight input digest for the ``/deep-module-review`` skill.

Migrated from ``backend/backend/aggregate/digest.py`` (issue #11 S3 / D12), per
#24 review finding F5: reuse the mature four-level truncation ladder and budget
control rather than rewriting.  Only *path + imports + ports* are fed to the
model -- never full-text excerpts.  The digest is derived entirely from a scan
Graph, so no filesystem reads happen here.

Adaptations for the skill (zero third-party dependency, #24 §5.3):
  - Pydantic removed: ``is_noise_module`` / ``NOISE_MARKERS`` are imported from
    ``metrics`` (single source of truth) instead of ``aggregate/validate.py``.
  - No default write path to the frontend manifest; ``analyze.py`` owns output.
  - Default budget stays ~40K chars (``API_TOTAL_DIGEST_CHARS``).

Budget & truncation ladder (deterministic):
...
model knows how much detail was elided.
"""
...
from metrics import is_noise_module

# Budgets. Local is conservative for a ~16K context (U3); the API gets a far
# larger budget. The skill reviews with Claude, so the authoritative 40K budget
# is the default (see build_digest).
